        ...
        <Link key={thread.id} href={`/thread/${thread.id}`}>
          <Card className="hover:border-primary transition-colors">
            <div className="space-y-2">
              {/* カテゴリ表示 */}
              {showCategory && (thread.category_name || thread.sub_category_name) && (
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                ...
                <span className="flex items-center gap-1">
                  <Clock size={16} />
                  {format(new Date(thread.created_at), 'yyyy/MM/dd HH:mm', { locale: ja })}
                </span>
                {thread.last_response_at && (
                  <span className="text-xs">
                    最終: {format(new Date(thread.last_response_at), 'MM/dd HH:mm', { locale: ja })}
                  </span>
                )}
              </div>
            </div>
