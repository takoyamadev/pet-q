    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <Link href={`/category/${categoryId}`}>
          <Card className={`px-4 py-2 cursor-pointer hover:border-primary transition-colors ${!subCategoryId ? 'border-primary bg-primary/5' : ''}`}>
            すべて
          </Card>
        </Link>
        {subCategories.map((subCategory) => (
          <Link key={subCategory.id} href={`/category/${categoryId}?sub=${subCategory.id}`}>
            <Card className={`px-4 py-2 cursor-pointer hover:border-primary transition-colors ${subCategoryId === subCategory.id ? 'border-primary bg-primary/5' : ''}`}>
              {subCategory.name}
            </Card>
          </Link>
        ))}
      ...
      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-4">
          {selectedSubCategory ? `${selectedSubCategory.name}のスレッド` : 'すべてのスレッド'}
        </h2>
