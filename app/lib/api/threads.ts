...
// 新着スレッド取得
export async function getLatestThreads(limit: number = 10): Promise<ThreadWithCategory[]> {
...
// スレッドIDで取得
export async function getThreadById(id: string): Promise<ThreadWithCategory | null> {
  const supabase = await createClient()
